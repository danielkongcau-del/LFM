...
class WormControlledUNetModel(UNetModel):
    """
    UNetModel with ControlNet-style multi-scale residual injection.

    The base architecture is unchanged. A companion adapter produces one
    control tensor for every encoder skip and one for the middle block. Those
    tensors are added before the decoder consumes the corresponding features.
    """
    ...
    def forward(self, x, timesteps=None, context=None, y=None, control=None, only_mid_control=False, **kwargs):
        ...
        controls = list(control) if control is not None else None
        ...
        if controls is not None:
            h = h + controls.pop().type(h.dtype)

        for module in self.output_blocks:
            skip = hs.pop()
            if controls is not None and not only_mid_control:
                skip = skip + controls.pop().type(skip.dtype)
...
class GatedZeroConv(nn.Module):
    """
    Zero-initialized residual projection with a learned spatial/channel gate.

    The residual path starts at exactly zero, so adding the adapter to an
    existing UNet does not perturb initial predictions. The gate is still
    learned per feature map rather than as a single scalar.
    """

    def __init__(self, channels, dims=2):
        super().__init__()
        self.residual = zero_module(conv_nd(dims, channels, channels, 1, padding=0))
        self.gate = zero_module(conv_nd(dims, channels, channels, 1, padding=0))

    def forward(self, x, emb=None, context=None):
        return self.residual(x) * th.sigmoid(self.gate(x))


class WormGatedMaskAdapter(nn.Module):
    """
    Full ControlNet-style adapter for latent mask conditioning.

    Unlike the original image-space ControlNet hint block, this adapter expects
    the mask condition to already be in latent resolution, e.g. [B, 4, 64, 64].
    It mirrors the UNet encoder and middle block, then emits gated residuals for
    every skip connection plus the middle feature.
    """
    ...
    def __init__(
        ...
        hint_hidden_channels = hint_hidden_channels or model_channels
        self.input_hint_block = TimestepEmbedSequential(
            conv_nd(dims, hint_channels, hint_hidden_channels, 3, padding=1),
            nn.SiLU(),
            conv_nd(dims, hint_hidden_channels, hint_hidden_channels, 3, padding=1),
            nn.SiLU(),
            zero_module(conv_nd(dims, hint_hidden_channels, model_channels, 3, padding=1)),
        )
        ...
        self.gated_zero_convs = nn.ModuleList([GatedZeroConv(model_channels, dims=dims)])
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                self.input_blocks.append(TimestepEmbedSequential(*layers))
                self.gated_zero_convs.append(GatedZeroConv(ch, dims=dims))
                self._feature_size += ch
            if level != len(channel_mult) - 1:
                out_ch = ch
                self.input_blocks.append(
        ...
        self.middle_block_out = GatedZeroConv(ch, dims=dims)
        self._feature_size += ch
    ...
    def convert_to_fp16(self):
        ...
        self.input_blocks.apply(convert_module_to_f16)
        self.middle_block.apply(convert_module_to_f16)
        self.gated_zero_convs.apply(convert_module_to_f16)
    ...
    def convert_to_fp32(self):
        ...
        self.input_blocks.apply(convert_module_to_f32)
        self.middle_block.apply(convert_module_to_f32)
        self.gated_zero_convs.apply(convert_module_to_f32)
    ...
    def forward(self, x, hint, timesteps, context=None, **kwargs):
        ...
        outs = []
        h = x.type(self.dtype)
        for module, gated_zero_conv in zip(self.input_blocks, self.gated_zero_convs):
            h = module(h, emb, context)
            if guided_hint is not None:
                h = h + guided_hint.type(h.dtype)
                guided_hint = None
            outs.append(gated_zero_conv(h, emb, context))
